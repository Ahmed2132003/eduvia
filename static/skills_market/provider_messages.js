const translations = {
    en: {
        ...
        "order-chat-service": "Chat with {{ order.buyer.username }} (Order: {{ order.service.title }})",
        "order-chat-opportunity": "Chat with {{ order.buyer.username }} (Opportunity: {{ order.opportunity_application.opportunity.title|slice:\"0:50\" }})",
        "order-status": "Status:",
    ...
    ar: {
        ...
        "order-chat-service": "الدردشة مع {{ order.buyer.username }} (الطلب: {{ order.service.title }})",
        "order-chat-opportunity": "الدردشة مع {{ order.buyer.username }} (الفرصة: {{ order.opportunity_application.opportunity.title|slice:\"0:50\" }})",
        "order-status": "الحالة:",
...
function toggleLanguage() {
    ...
    document.querySelectorAll('[data-translate]').forEach(element => {
        ...
        if (element.tagName.toLowerCase() === 'meta') {
            ...
            if (key === 'order-chat-opportunity' && !text.includes('No details available')) {
                text = text.replace('{{ order.opportunity_application.opportunity.title|slice:"0:50" }}', element.textContent.match(/Opportunity: (.*)\)/)?.[1] || 'No details available');
            }
